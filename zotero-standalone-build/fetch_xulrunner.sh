...
CALLDIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"  # 获取绝对路径
. "$CALLDIR/config.sh"  # 读配置文件
...
function modify_omni {
	...
	python3 "$CALLDIR/scripts/optimizejars.py" --deoptimize ./ ./ ./  # 解除优化
	unzip omni.ja
	rm omni.ja
	
	# Modify various constants (e.g., to allow unsigned add-ons) 允许未签名的插件——比如 Zotero 本体
...
# Add devtools server from browser omni.ja devtools 服务器？？？？？
...
if [ $BUILD_MAC == 1 ]; then
	...
	##########
	# 篡改核心文件 & 添加 devtools 服务器，最核心操作，下面两者即针对 win32 与 linux 配置，原理相同
	modify_omni mac
	extract_devtools
	##########
	popd  # 切目录
